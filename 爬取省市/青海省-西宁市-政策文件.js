...
async function t2(url) {
    ...
    let urlList = []
    //------
    $('.cm-news-list a').each((index, element) => {
        let urlaa = $(element).attr('href').replace(/^\.\//, '') || '';
        if (urlaa !== '') {
            urlList.push(url +urlaa)
        }
    })
...
// t1('https://www.xining.gov.cn/zwgk/zc/xzgfxwj/')

// t2('https://www.xining.gov.cn/zwgk/zc/xzgfxwj/')
...
module.exports = {
    ...
    location: '青海省-西宁市',
    tag: "政策文件",
    homeUrl: 'https://www.xining.gov.cn/zwgk/zc/xzgfxwj/'
}   